...
class FakeStockPortfolio:
    ...
    def get_current_price(self, ticker: str) -> float:
        ...
        if res == 0:
            raise StockPortfolioError("Could not get price.")
        return res
    ...
    def sell(self, ticker: str, amount: float):
        if amount <= 0:
            raise StockPortfolioError("Cannot sell zero or negative stock")
...
def main():
    ...
    if finnhub_api_key is None:
        raise APIKeyNotFoundError("Cannot find FINNHUB_API_KEY environment variable.")
